...
>(
  "user/login",
  async (
    { username, password, from, navigate, setToast },
    { rejectWithValue }
  ) => {
    try {
...
export const refresh = createAsyncThunk<User, {}, { rejectValue: string }>(
  "user/refresh",
...
>(
  ...
  async ({ selectedIds, setToast }, { rejectWithValue }) => {
    try {
      ...
      setToast({
        open: true,
        message: "Doctor types updated successfully!",
        type: "success",
      });
      return user;
    } catch (error) {
      setToast({
        open: true,
        message: "Failed to update doctor types!",
        type: "error",
      });
      return rejectWithValue(error.toString());
    }
